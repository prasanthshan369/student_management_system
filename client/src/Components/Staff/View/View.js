...
const handleUpdate = async (e) => {
  const { data } = await axios.post("/staff/read", { _id:e.target.value });
...
const handleDelete=async(e)=>{
  const{data}=await axios.post('/staff/delete',{_id:e.target.value })
  hanldeLoad()
  ...
  return (
    ...
    {datas.map((datas, i) => (
     ...
     <img src={datas.staff_Image} className="view__image" />
     <div className="view__content">
       <div className="flex">
         <h3 className="flex_text">{datas.staff_Name}</h3>
         <p className="flex_text"> {datas.staff_Number}</p>
       </div>
       <div className="flex">
         <p className="flex_text">Exp: {datas.staff_Exp}</p>
         <p className="flex_text">
          {
            datas.course ?(datas.course.course_Name):"No Course"
          }
         </p>
